...
```
网页做完，{N} 章 {M} 步，dev server 在 localhost:5174 跑着（端口由 vite.config.ts 的 server.port 指定，脚手架默认 5174）。
...
Part 8「常见用户反馈速查」。**关键**：先定位是哪一层（节奏 / 视觉 / 内容
/ 代码），再改最小切片，**不要重做整章**。

---

## ⚠️ Checkpoint Plan 沟通陷阱（多次踩过的坑）

Checkpoint Plan 是硬节点没错，但**实施方式**决定体验。本节记录踩过的坑，避免下次复现。

### 陷阱 1：把"5 件事"做成 5 个独立问题轰炸用户

最常见的失败是 Checkpoint Plan 一次性把稿子 / outline / 主题 / 素材 / 模式 **5 个问题摊开** 等用户逐项回答。**用户此时只想确认一次性前进** —— 看到 5 个 open question 会烦。

**正确做法**：
- 第 1 次 Checkpoint：展示完整稿子 + 完整 outline + 自动推荐一个主题 + 推荐一个开发模式 + 默认素材模式，**用户只要回答"OK"或指出要改哪里**
- 后面如果用户说"主题 electric-studio" "素材你去找" "模式 B"，就**逐项收束**，别每次都重新把所有问题摊出来
- 已经回答过的项不要再问

### 陷阱 2：检测不到网络中断 / 用户在测试连通性

用户连发 "你好" / "1" / "nihao" / "测试" 这种**短消息**时，往往是：
- 网络中断后回来的握手信号（之前的请求可能丢失了）
- 单纯测试连通性
- 上一轮说的不清楚再来一次
...
- **不要**把这当作用户想要开始新任务
- **不要**重复推进 Phase 1 内容
- **要**简洁确认："刚才在做什么？继续还是新开？"
- 如果之前有未完成的工具调用 / 被打断的会话，**主动**给一份"中断前的状态摘要 + 等用户回复如何继续"，而不是反过来轰炸确认清单

### 陷阱 3：dev server 卡在 Vite 缓存或浏览器状态

第 1 章验收时常见症状：dev server 起来了，浏览器还在显示脚手架自带的示例章节。
...
pkill -f vite
# 2. 清 Vite 缓存（最常见）
rm -rf presentation/node_modules/.vite presentation/dist
# 3. 重启 dev server
cd presentation && npm run dev
# 4. 浏览器 hard-reload（不要用软刷新）
```

**不要**：反复猜测浏览器缓存、Vite 默认配置、HMR 问题；直接清缓存重启。

### 陷阱 4：第 1 章代码写到一半遇到 syntax 错误

`write_file` 的字符串里如果有未闭合的引号、字符串拼接转义错位，整个 `.tsx` 文件会编译报错。**`npx tsc --noEmit` 在每章写完后立即跑**（不依赖 dev server），发现错误立刻回看最后一次 write 的内容，**不要**继续往下写。

### 陷阱 5：写完所有章节后才发现 dev server 验证失败 → 重写浪费时间

**预防**：
- 第 1 章写完 → 清缓存重启 dev server → 浏览器验证 → **用户验收** ✅
- 模式 B / C 一次性做第 2~N 章之前，必须**先验证脚手架 / 主题 token / browser console / 无 ts error** 这些基础设施，再开始批量写

### 陷阱 6：用户因验证环节卡住而终止整个项目（必须避免）

**踩坑场景**：第 1 章写到中间 → dev server 反复起不来 / 浏览器显示旧示例章 / 工具调用开始失败 → 用户连发 "1" "nihao" "小空" "上面缓存内容" → 用户主动说 "上面的任务我停止那个"（即终止整个项目）。

**根因**：第 1 章**强制定位成 anchor**，每次修复都默认重新启 dev server、浏览器刷新、看一屏。如果 anchor 一直卡在"环境问题"（容器断电、token 缓存丢失、Vite 缓存旧化），用户在第 1 章上花的耐心很快耗尽 → 不是"第 1 章写得不好"，而是"基础设施 + 用户协作同时断"。

**预防**：
1. **基础设施先验收再开 chapter 1**（详见 FIRST-USE.md）—— scaffold 跑完立刻验证空白脚手架能跑起来（npm install + npm run dev + 浏览器访问默认示例章都正常）然后再开始写第 1 章。这一步通常 30 秒，能省后续半小时的循环
2. **关键缓存要在工作目录外**：
   - API token / PAT → `/tmp/_xxx_b64.txt`（base64 文件，**不是** `/tmp/web-video/.../` 下）
   - 容器侧：`/opt/data/home/` 是 1Panel 容器挂载点，**重启会清空** `/tmp/`，所以 `/tmp/_htok_b64.txt` 这类需要持久化的 token 不能放这里
   - 工作目录 `/tmp/web-video/<project>/` 放 `presentation/`、`article.md`、`script.md`、`outline.md` 即可，token 不要进这里
3. **STORAGE_KEY 版本号必须 bump**：每次 `chapters.ts` 改 → bump `presentation/src/hooks/useStepper.ts` 里的 `STORAGE_KEY`（`v4` → `v5`），否则旧用户会话的 localStorage 游标会指向不存在的 step 而永远卡住
4. **写每章 .tsx 时防御引号转义错位**：用 `write_file` 写 `const composeYaml = [...]` 这种数组时，**特别留意每行末尾是不是 `" ,`** 还是只 `",`** —— 我自己曾经一行少了闭合 `",]`，TS 编译错误立即出来。策略：**写完单个 `.tsx` 立刻 `npx tsc --noEmit` 检查**，别等所有章节写完一起检查
5. **用户说"终止 / 停止 / 算了"时立即收手** —— 不要追问"是否保留 / 清理一下 / 切换模式"，直接 **stop + 把已完成的状态简短汇报**（"已经写到第 X 章 / skill 已写好放在 Y / token Z 等"），让用户决定是否继续 / 重开 / 清理

### 陷阱 7：Checkpoint Plan 之后的开发，与 Phase 1 写作期是不同节奏

| 阶段 | 用户的耐心 | 期望的 agent 行为 |
|---|---|---|
| Phase 1 写作期（script + outline） | 高 | 等反馈、改到满意才进 Phase 2 |
| Checkpoint Plan | 中 | 一次问 5 件；不要逐件问 |
| Phase 2 开发期 | 低 | 不用每章都问；按选定模式走；如果卡住超过 2 轮，先汇报"卡在哪 + 我打算怎么试"，而不是闷头试 |

--

---
## ⚠️ 部署 & 远程访问（用户无法访问 localhost 时）

当 agent 运行在容器或远程机器上（如 Hermes Agent 容器），用户无法直接访问 `localhost:5174`。在这种情况下，需要在 Phase 2 之后先部署构建产物到 VPS 或用户可访问的服务器，再让用户验收。
...
# 6. 添加 Cloudflare DNS 记录（如有）
# A 记录：pres → VPS_IP（代理开启）
```

### ⚠️ 关键注意事项

- **Vite dev server 端口**：脚手架 `vite.config.ts` 默认端口为 **5174**（不是 5173）。告知用户时写对端口号。
- **容器端口映射**：如果 agent 运行在 Docker 容器中，容器内启动的 dev server 需要容器端口映射才能从宿主机访问。Hermes Agent 容器默认只暴露 9119 端口，因此 dev server 无法从外部直接访问。
- **构建后部署**：用 `npx vite build` 构建后，在 VPS 上通过 Python `http.server` 或 Nginx 静态文件服务可快速让用户访问。部署到 VPS 后需配置 Nginx 反代和（可选）Cloudflare DNS。
- **临时域名**：用一个子域名（如 `pres.example.com`）临时指向部署，验收通过后可删除 DNS 记录和 Nginx 配置。
- **清理**：验收/演示完成后，记得停止 HTTP 服务器并删除临时 Nginx 配置和 DNS 记录。
- **SSH 密码部署**：当容器无 SSH key 且仅密码认证可用时，使用 `pexpect` 库自动化密码输入（见下方替代方案）。
- **用户偏好"一次做完"**：如果用户在第 1 章验收后说"一起做"或"剩下的一起做"，直接走 **模式 B（顺序开发）** 或 **模式 C（并行开发）**，不再逐章暂停确认。

### SSH 密码部署替代方案（当 SSH key 不可用时）
...
HOST = "your.vps.ip"
PORT = 22
PASSWORD = "your-password"  # 或通过 sys.argv 传入

# SCP 部署包
child = pexpect.spawn(f"scp -o StrictHostKeyChecking=no -P {PORT} /tmp/pres-demo.tar.gz root@{HOST}:/tmp/")
...
child = pexpect.spawn(
    ...
    f"cd /opt/presentation-demo && rm -rf * && tar xzf /tmp/pres-demo.tar.gz -C /opt/presentation-demo"
)
child.expect("password:")
child.sendline(PASSWORD)
child.expect(pexpect.EOF, timeout=30)
```
- **SSH 口令认证**：如果 VPS 仅支持口令而非密钥认证（或 SSH 端口非标准），参考 [`references/DEPLOYMENT.md`](references/DEPLOYMENT.md) 的「SSH 口令认证替代方案」章节，使用 Python pexpect 实现交互部署。
...
| [`references/FIRST-USE.md`](references/FIRST-USE.md) | **首次使用 / 新手快速上手** | Scaffold 后必做清单（删示例章、bump STORAGE_KEY、注册章节）、常见端口问题、素材截图工作流 |
